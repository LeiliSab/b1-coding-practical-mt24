...
class Mission:
    ...
    def from_csv(cls, file_name: str):
        ...
        reference = df.iloc[:, 0].to_numpy()  # First column
        cave_height = df.iloc[:, 1].to_numpy()  # Second column
        cave_depth = df.iloc[:, 2].to_numpy()  # Third column .to_numpy() method converts the selected columns to NumPy arrays.
        return cls(reference, cave_height, cave_depth)
